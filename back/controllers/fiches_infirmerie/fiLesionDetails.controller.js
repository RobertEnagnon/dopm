...
exports.getAllCategory = async (req, res, next) => {
  await prisma.fi_lesiondetails
    .findMany({})
    .then((result) => {
      res.status(201).json(result);
    })
    .catch((error) => {
      console.log('error lesionDetails', error);
      res.status(201).json({
...
exports.createCategorie = async (req, res, next) => {
  ...
  const newCategory = await prisma.fi_lesiondetails.create({
    data: {
      name: fiCategoryName
    }
  })
...
exports.updateCategory = async (req, res, next) => {
  ...
  const researchFiCategory = await prisma.fi_lesiondetails.findUnique({
    where: {
      id: parseInt(req.params.id),
    },
  })
  if (researchFiCategory) {
    const updateCategory = await prisma.fi_lesiondetails
      .update({
        where: {
          id: parseInt(req.params.id),
        },
...
exports.deleteCategory = async (req, res, next) => {
  ...
  const researchCategory = await prisma.fi_lesiondetails.findUnique({
    where: {
      id: parseInt(req.params.id),
    },
  })
  if (researchCategory) {
    await prisma.fi_lesiondetails.delete({
      where: {
        id: parseInt(req.params.id),
      },
    })
