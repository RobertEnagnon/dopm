...
exports.getAllCategory = async (req, res, next) => {
  await prisma.fi_materialelements
    .findMany({})
    .then((result) => {
      res.status(201).json(result);
    })
    .catch((error) => {
      console.log('error materialElements', error);
      res.status(201).json({
...
exports.createCategorie = async (req, res, next) => {
  ...
  const newCategory = await prisma.fi_materialelements.create({
    data: {
      name: fiCategoryName
    }
  })
...
exports.updateCategory = async (req, res, next) => {
  ...
  const researchFiCategory = await prisma.fi_materialelements.findUnique({
    where: {
      id: parseInt(req.params.id),
    },
  })
  if (researchFiCategory) {
    const updateCategory = await prisma.fi_materialelements
      .update({
        where: {
          id: parseInt(req.params.id),
        },
...
exports.deleteCategory = async (req, res, next) => {
  ...
  const researchCategory = await prisma.fi_materialelements.findUnique({
    where: {
      id: parseInt(req.params.id),
    },
  })
  if (researchCategory) {
    await prisma.fi_materialelements.delete({
      where: {
        id: parseInt(req.params.id),
      },
    })
