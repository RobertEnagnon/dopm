...
const InjuredCategoryComp = () => {
  const { categories, addCategory, updateCategory, deleteCategory } = useCategory<InjuredCategory>({endpoint: 'fi_injcategories'})
  const [openAdd, setOpenAdd] = useState<boolean>(false)
  const [openEdit, setOpenEdit] = useState<boolean>(false)
  const [openDelete, setOpenDelete] = useState<boolean>(false)
  const [selectedCategoryId, setSelectedCategoryId] = useState<number>()
  const [selectedCategory, setSelectedCategory] = useState<InjuredCategory>()
      ...
      <Row className="mb-4 align-items-end">
        <Col>
          <h6 className="card-subtitle text-muted">
            Gestion des <code>paramètres d'identification de blessés</code>
          </h6>
        </Col>
            ...
            categories.map((category, index) => {
              return (
                <tr key={index}>
                  <td>{category.id}</td>
                  <td>{category.name}</td>
                  <td>{category.isInjuredCategoryName === true? 'oui': 'non'}
                  </td>
                  <td>
                    {category.createdAt
                        ? moment(
...
export default (InjuredCategoryComp)
